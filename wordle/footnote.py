...
class Confessions(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(*(FadeIn(title, RIGHT) for title in titles), lag_ratio=0.3)
        )
        self.wait()
    ...
    def construct(self):
        ...
        self.play(
            self.get_student_changes(
                "raise_left_hand", "pondering", "raise_right_hand"
            ),
...
class HowWordleColoringWorks(WordleScene):
    def construct(self):
        ...
        self.play(
            Write(answer_row.target, remover=True), Write(answer_label), run_time=1
        )
...
class MatrixOfPatterns(Scene):
    ...
    def construct(self):
        ...
        words.sort()
        word_mobs = VGroup(
            *(Text(word, font="Consolas", font_size=30) for word in words)
        )
            ...
            anims.extend(
                [
                    bar.animate.set_height(
                        value * height, stretch=True, about_edge=DOWN
                    ),
...
class SaletSecondGuessMap(HowLookTwoAheadWorks):
    ...
    def construct(self):
        path = os.path.join(
            get_directories()['data'], 'wordle', 'salet_with_brute_force_guess_map.json'
        )
        ...
        arrows, guess2s = self.get_next_guess_group(
            guess1.text, pattern_array1, possibilities
        )
...
class RankingUpdates(Scene):
    def construct(self):
        ...
        # One step
        with open(
            os.path.join(get_directories()["data"], "wordle", "best_entropies.json")
        ) as fp:
            rows1 = VGroup()
        ...
        with open(
            ...
            for word, score, dist in json.load(fp)[:n_shown]:
                row = VGroup(
                    ...
                    Tex("\\rightarrow"),
                    DecimalNumber(
                        score / len(answers), num_decimal_places=3, color=BLUE
                    ),
                )
                row.arrange(RIGHT)
                rows3.add(row)
        ...
        col1, col2, col3 = cols = [
            get_column(rows, title) for rows, title in zip(all_rows, titles)
        ]
...
class ForgetTheBestWord(Scene):
    def construct(self):
        ...
        self.play(
            FadeOut(randy.bubble, scale=0.25, shift=2 * DL), randy.animate.change("erm")
        )
        self.wait()
