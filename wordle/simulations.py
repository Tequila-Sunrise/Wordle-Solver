...
def brute_force_optimal_guess(
    all_words, possible_words, priors, n_top_picks=10, display_progress=False
):
    ...
    if display_progress:
        iterable = ProgressDisplay(
            top_choices, desc=f"Possibilities: {len(possible_words)}", leave=False
        )
    else:
...
def find_top_scorers(n_top_candidates=50):
    ...
    file = os.path.join(
        get_directories()["data"], "wordle", "best_double_entropies.json"
    )
    with open(file) as fp:
...
def find_best_two_step_entropy():
    ...
    double_ents = [
        [top_candidates[i], top_ents[i], ents2[i]] for i in sorted_indices2[::-1]
    ]

    ent2_file = os.path.join(
        get_directories()["data"], "wordle", "best_double_entropies.json"
    )
...
def get_optimal_second_guess_map(first_guess, n_top_picks=10, regenerate=False):
    ...
    for pattern, bucket in ProgressDisplay(list(enumerate(buckets)), leave=False):
        sgm[pattern] = brute_force_optimal_guess(
            all_words, bucket, priors, n_top_picks=n_top_picks, display_progress=True
        )
